...
function vizMapper(cols, lookup, colNum, type) {
  ...
  for (let i = 0; i < items.length; i++) {
    try {
      if ('secondaryInfo' in items[i].json) {
        ...
        const dataColumns = root.dataDictionary.presModelHolder.genDataDictionaryPresModel.dataSegments['0'].dataColumns;
        const dataLookup = new Map(dataColumns.map((d) => [d.dataType, d.dataValues]));
